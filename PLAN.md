...
| InnoSign.eu | Video + subtitles | ✅ Permission granted | SPJ videos with subtitles — get formal written agreement |
| [reference dictionary] | Multilingual dictionary | ✅ Via [partner organization] | SVK content produced by [partner organization]; use with [partner organization] written permission (platform ToS does not override creator rights) |
| [vocabulary reference] | Online dictionary | ⚠️ Proprietary ([vocabulary organization]) | ~9,000 signs; vocabulary reference only — ML training requires [vocabulary organization] agreement |
...
| InnoSign.eu videos | ✅ Yes (get in writing) | ✅ With agreement | ✅ Yes |
| [reference dictionary] | ✅ Via [partner organization] written permission | ✅ With agreement | ✅ Yes |
...
- **Dictio** (Teiresias Centre, Masaryk University) — multilingual; integrates SignWriting + HamNoSys; free; use as a **notation learning reference** only — annotators study it to understand the system (hand shapes, location, movement encoding), then write original SPJ entries from scratch. This is NOT copying; SignWriting is a universal notation system like musical notation.
- **SignPuddle Online** — community sign dictionaries including SPJ entries; free; reference for SPJ-specific notation conventions already established by the community
...
Own SPJ videos (~300) + Museum content (100h+)   ← fully owned
    ...
    + InnoSign.eu videos (with subtitles)          ← permission granted
    + [reference dictionary] (via [partner organization] permission)← permission from creator
    ...
    Better AI suggestions → faster annotation loop
         ↓
    ID-gloss annotations → SignWriting (annotators write original SPJ entries using Dictio/SignPuddle as notation guide)
         ↓
    SPJ SignWriting dictionary (no ML training required for this step)
```
...
- [ ] Inventory and assess own corpus assets: ~300 SPJ videos (with Slovak subtitles) + 100+ hours museum SPJ content + [partner organization] videos + InnoSign.eu videos — assess annotation coverage, extract MediaPipe pose as first training pool
- [ ] Formalize data-sharing agreements with [partner organization] and InnoSign in writing (scope: ML training, derivative datasets, publication) — verbal permission already granted but written agreement needed for corpus deposit and publication
...
6. SignWriting (optional — annotators create original SPJ entries using Dictio/SignPuddle as a notation learning guide; no separate ML training required)
...
- [ ] Publish SPJ SignWriting dictionary — original entries created during Phase 3 annotation using Dictio/SignPuddle as notation reference; no training required
