"""Inference pipeline: gloss prediction, EAF annotation, and timeline visualisation.

Takes a trained PoseTransformerEncoder, runs it on detected sign segments from
new videos, writes predictions to EAF AI tiers, and provides timeline visualisation.
"""
...
def predict_segments(
    ...
    Returns:
        List of dicts with keys:
            ...
            predicted_gloss, prediction_confidence, top3_glosses
    """
    if device is None:
    ...
    # Sort by start time
    predictions.sort(key=lambda p: p["start_ms"])
    return predictions


def write_predictions_to_eaf(
    ...
    and AI_Confidence with prediction confidence scores.

    Args:
        predictions: Output of predict_segments().
        eaf_path: Path to EAF file to update.
        overwrite: If True, clear existing AI annotations first.

    Returns:
        {'rh_predictions': int, 'lh_predictions': int}
    """
    ...
    for pred in predictions:
        ...
        end_ms = pred["end_ms"]
        conf = str(round(pred["prediction_confidence"], 3))
        hand = pred["hand"]
...
def read_predictions_from_eaf(eaf_path: Path) -> list[dict]:
    ...
    Returns:
        List of dicts sorted by start_ms, same schema as predict_segments():
            {hand, start_ms, end_ms, predicted_gloss, prediction_confidence}
    """
    ...
    for tier, hand in tier_hand_map.items():
        ...
        for start, end, value, *_ in eaf.get_annotation_data_for_tier(tier):
            ...
            predictions.append({
                ...
                "prediction_confidence": _find_confidence(start_ms, end_ms),
            })
...
def predictions_timeline_figure(
    ...
    Returns:
        Plotly Figure with horizontal bars for each prediction.
    """
    import plotly.graph_objects as go

    if not predictions:
        fig = go.Figure()
        fig.update_layout(title="No predictions", height=200)
        return fig

    # Separate by hand
    rh_preds = [p for p in predictions if p["hand"] == "right"]
    lh_preds = [p for p in predictions if p["hand"] == "left"]
