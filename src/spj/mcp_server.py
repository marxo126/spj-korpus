...
def spj_run_inference(video_filename: str, checkpoint_filename: str) -> dict:
    """Run gloss prediction on a video using a trained checkpoint.

    Writes predictions to the video's EAF file (AI tiers).
    ...
    try:
        ...
        write_predictions_to_eaf(predictions, eaf_path)
            ...
            "predictions": [
                {
                    ...
                    "confidence": round(p["prediction_confidence"], 3),
                    "hand": p["hand"],
                    "start_ms": p["start_ms"],
                    "end_ms": p["end_ms"],
                }
                for p in predictions[:20]  # Limit to first 20 for readability
            ],
        }
