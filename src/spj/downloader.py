...
Supported download sources:
  ...
  - Dictionary CSV (bulk individual sign videos) via yt-dlp
...
Usage:
    ...
    from spj.downloader import load_dictionary_csv, download_dictionary_batch
...
# ================================================================== #
#  Dictionary CSV download (bulk individual sign videos)
# ================================================================== #
...
def load_dictionary_csv(csv_path: Path) -> pd.DataFrame:
    """Load a dictionary export CSV for bulk video download.
    ...
    Args:
        csv_path: Path to the dictionary CSV export.
...
def download_dictionary_video(
    row: pd.Series,
    output_dir: Path,
    yt_dlp_cmd: str = "yt-dlp",
    skip_existing: bool = True,
) -> dict:
    """Download a single dictionary video entry.
...
def download_dictionary_batch(
    ...
    """Download dictionary videos in parallel from a DataFrame.
    ...
    def _worker(row: pd.Series) -> dict:
        result = download_dictionary_video(row, output_dir, yt_dlp_cmd, skip_existing)
