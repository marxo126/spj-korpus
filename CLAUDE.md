...
3. Use **Dictio** (Teiresias/Masaryk) and **SignPuddle** as notation learning guides only — they are references, not sources to copy

**Dictio and SignPuddle are notation references like a musical score — you study them to
learn the system, then compose original SPJ entries from scratch.**

Output: SPJ SignWriting dictionary built as a by-product of Phase 3 annotation. No ML
...
Confirm all three:
2. Written data-sharing agreement signed with **InnoSign** (same scope)
...
| `src/spj/inference.py` | `predict_segments()`, `write_predictions_to_eaf()`, `predictions_timeline_figure()` |
...
| `src/spj/spreadthesign.py` | `load_dictionary()`, `filter_conversational()`, `download_one()`, `download_batch()`, `save_manifest()` |
