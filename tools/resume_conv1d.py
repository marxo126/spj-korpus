"""Resume Conv1D training for unified_2plus and unified_3plus only.

Continues from saved checkpoints. reference-dict_2plus dropped (too few samples/label).

Usage:
    nohup .venv/bin/python tools/resume_conv1d.py > data/resume_conv1d.log 2>&1 &
"""
import logging
import os
import signal
...
def run_experiment(name: str, splits_dir: Path, output_dir: Path):
    ...
    print(f"\n[{name}] Resuming: {len(train_df)} train, {len(val_df)} val, "
...
def main():
    print("=" * 60)
    print("  Resume Conv1D Training (unified_2plus + unified_3plus)")
    print("=" * 60)
    ...
    experiments = [
        ...
        {
            "name": "unified_2plus",
            "splits_dir": DATA_DIR / "training" / "splits_unified_2plus",
    ...
    for exp in experiments:
        t = threading.Thread(target=run_experiment, kwargs=exp, name=exp["name"])
        threads.append(t)
        t.start()
        time.sleep(2)
    ...
    print(f"\n{'=' * 60}")
    print(f"  Done in {total / 3600:.1f} hours")
    print(f"{'=' * 60}")
