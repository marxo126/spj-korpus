#!/usr/bin/env python3
"""Split category-source dual-view videos and extract poses for each half.

category-source videos (1280x720 or 1280x556) contain two camera angles:
  - Left half  (cols 0-631):   ~60 deg frontal view
  - Right half (cols 648-1279): ~90 deg profile view
  - Dark divider at cols 632-647

This script reads inventory.csv, filters to source=category-source, checks which
videos already have _60.pose and _90.pose files, and extracts dual-view poses
for the rest.

Usage:
    .venv/bin/python tools/split_category-source_poses.py
    .venv/bin/python tools/split_category-source_poses.py --workers 4 --backend gpu
    .venv/bin/python tools/split_category-source_poses.py --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract dual-view poses from category-source videos",
    )
    parser.add_argument(
    ...
    # Filter to category-source — identified by 'category-source' in the path
    if "source" in df.columns:
        kod = df[df["source"].str.lower() == "category-source"].copy()
    elif "path" in df.columns:
        kod = df[df["path"].str.contains("category-source", case=False, na=False)].copy()
    else:
        print("ERROR: inventory.csv has neither 'source' nor 'path' column")
        sys.exit(1)

    print(f"category-source videos in inventory: {len(kod)}")

    if kod.empty:
        print("No category-source videos found. Nothing to do.")
        return
    ...
    if "path" in kod.columns:
        ...
        video_dir = DATA_DIR / "videos" / "category-source"
