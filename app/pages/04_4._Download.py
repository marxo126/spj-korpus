"""Download page — YouTube, dictionary CSV, FTP, and direct HTTP downloads.
...
from spj.downloader import (
    ...
    load_dictionary_csv, download_dictionary_batch,
...
- **Dictionary CSV** — bulk download individual sign videos from a CSV export
...
tab_yt, tab_dict, tab_ftp, tab_http = st.tabs([
    "🎬 YouTube", "📖 Dictionary CSV", "📁 FTP", "🌐 HTTP"
])
...
with tab_dict:
    st.markdown(
        "Download individual sign videos from a dictionary CSV export. "
        "Each video = one sign (~2-5s)."
    )

    _DICT_CSV = DATA_DIR / "Dictionary_export.csv"
    _DICT_DIR = VIDEO_DIR / "dictionary"

    # Look for any Dictionary_export*.csv in data/
    _dict_candidates = sorted(DATA_DIR.glob("Dictionary_export*.csv"))
    if _dict_candidates:
        _DICT_CSV = _dict_candidates[-1]  # use most recent

    if _DICT_CSV.exists():
        dict_df = load_dictionary_csv(_DICT_CSV)
        st.caption(f"CSV: `{_DICT_CSV.name}` — {len(dict_df)} entries")

        # Agreement check
        if not check_agreement(_DICT_DIR):
            st.warning(agreement_warning("dictionary"))
        ...
        if st.button("📥 Download Dictionary Videos", type="primary", disabled=len(filtered) == 0):
            dl_df = filtered.copy()
            if dict_limit > 0:
            ...
            results = download_dictionary_batch(
                dl_df, _DICT_DIR,
        ...
        st.info(
            "No dictionary CSV found. "
            "Place `Dictionary_export*.csv` in `data/` to enable."
        )
