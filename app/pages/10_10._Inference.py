...
Tab 2 (Preview):  Video player + timeline + predictions table
"""
import sys
...
4. Open the EAF file in ELAN to review AI predictions.
5. Corrections flow back to training data via page 7 → retrain on page 8.

**Tab 👁 Preview**
- View predictions timeline overlaid on the video.
                ...
                if run_selected or run_batch:
                    ...
                    for i, stem in enumerate(target_stems):
                        ...
                        try:
                            ...
                            # Write predictions to EAF
                            write_result = write_predictions_to_eaf(
                                predictions, eaf_path, overwrite=True,
                            )
                            ...
                            all_results.append({
                                ...
                                "rh_predictions": write_result["rh_predictions"],
                                "lh_predictions": write_result["lh_predictions"],
                                "status": "OK",
                            })

                            # Cache predictions for preview
                            st.session_state[f"inf_preds_{stem}"] = predictions
...
with tab_preview:
    ...
    if not results:
        ...
        if not ok_videos:
            st.warning("No successful predictions to preview.")
        else:
            ...
            if selected_preview:
                predictions = st.session_state.get(f"inf_preds_{selected_preview}", [])
                ...
                if not predictions:
                    ...
                    # Timeline
                    from spj.inference import predictions_timeline_figure

                    st.subheader("Predictions Timeline")
                    timeline_fig = predictions_timeline_figure(predictions, duration_sec)
                    ...
                    for p in predictions:
                        ...
                        pred_rows.append({
                            ...
                            "Confidence": f"{p['prediction_confidence']:.0%}",
                            "Top 3": top3_str,
                        })
                    ...
                    if not backends:
                        ...
                        if model_name:
                            ...
                            if tc1.button("Translate predictions", key="do_translate"):
                                if is_mlx:
                                    from spj.mlx_translate import batch_translate_predictions as mlx_batch
                                    with st.spinner(f"Translating via MLX ({model_name.split('/')[-1]})..."):
                                        translated = mlx_batch(
                                            predictions, model_id=model_name,
                                        )
                                else:
                                    from spj.ollama_translate import batch_translate_predictions as ollama_batch
