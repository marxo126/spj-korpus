...
st.header("12. 🔍 AI Prepartner-dictn Review")
st.caption(
...
# Trim values — check persisted trims first, fall back to original prepartner-dictn bounds
...
if _pose_loaded:
    try:
        # Find cluster of nearby prepartner-dictns for stable switching
        cluster_start, cluster_end = _find_prepartner-dictn_cluster(preds, current_idx)
        ...
        if video_b64:
            ...
            # Restore persisted cut points for this prepartner-dictn
            ...
            if result is not None:
                ...
                if (_tl_sel == current_idx and
                    ...
                    # Persist trim for this prepartner-dictn
                    trims = st.session_state.setdefault("_ar_trims", {})
                    trims[current_idx] = (_use_start_ms, _use_end_ms)
                # Persist cut points for this prepartner-dictn (cap at 20 entries)
    ...
    with c_save:
        if st.button("Save All Segments", type="primary", use_container_width=True):
            ...
            if _all_ok and _resolved:
                ...
                p_fps_val = vid["fps"]
                hand = pred["hand"]
                # Only annotate trim if overall bounds differ from prepartner-dictn
                _pred_trimmed = (
...
def _advance_after_save() -> None:
    """Clear persisted trim and advance to next pending prepartner-dictn."""
...
# Mouthing row — set default from existing pairing when prepartner-dictn changes
