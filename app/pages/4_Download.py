...
# ------------------------------------------------------------------ #
# SpreadTheSign dictionary download
# ------------------------------------------------------------------ #
st.divider()
st.subheader("📖 SpreadTheSign Dictionary Download")
st.caption(
    "Download individual sign videos from spreadthesign.com using their "
    "dictionary CSV export. Each video = one sign (~2-5s, 320x240)."
)

STS_CSV = DATA_DIR / "Dictionary_export_2026-02-26_10-01-57.csv"
STS_DIR = VIDEO_DIR / "spreadthesign"

if STS_CSV.exists():
    from spj.spreadthesign import load_dictionary, download_batch, _sanitize_filename

    sts_df = load_dictionary(STS_CSV)
    ...
    if st.button("📥 Download SpreadTheSign", type="primary", disabled=len(filtered) == 0):
        dl_df = filtered.copy()
        if sts_limit > 0:
    ...
    st.info(
        "No SpreadTheSign dictionary CSV found. "
        "Place `Dictionary_export_*.csv` in `data/` to enable."
    )
