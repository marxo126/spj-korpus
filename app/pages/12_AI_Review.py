...
and approve / correct / skip with one click. Approved predictions write directly
...
st.header("AI Prediction Review")
st.caption(
...
with st.expander("Predictions Timeline", expanded=True):
    ...
    fig = predictions_timeline_figure(preds, duration_sec)
...
if "_ar_nav_target" in st.session_state:
    ...
    # Reset trim sliders for new prediction
...
# Reset trim sliders if prediction changed (selectbox or any other way)
...
# Context: show wider window around the prediction
...
if _pose_loaded:
    ...
    if ctx_speed is not None and len(ctx_speed) > 0:
        ...
        if n_bars > 0:
            ...
            # Mark original prediction region
...
def _write_pairing(word: str, gloss_id: str, status: str, note: str) -> None:
    ...
    # Use trimmed boundaries (may differ from original prediction)
...
def _resolve_words_and_gloss() -> tuple[str, str]:
    ...
    Input field supports:
      - Empty → approve AI prediction as-is
...
with act_approve:
    # Single "Save" action: if gloss field is empty → approve AI prediction as-is,
